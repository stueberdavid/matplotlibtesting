...
# Filter aktivieren
def set_alternate_packet_loss(device):
    # tc-Befehl zur alternierenden Paketverlust-Konfiguration
    command = [
        "tc", "qdisc", "add", "dev", device, "root", "netem",
        "loss", "50%",  # 50% Paketverlust
    ]
...
# Beispielaufruf
set_alternate_packet_loss(device="enp0s25")


# Variablen setzen
INTERFACE = "enp0s25"
#INTERFACE = "eno1"
CAPTURE_FILE = "/home/david/test.filter.pcap"
LOG_FILE = "/home/david/receiver_resource_usage.filter.csv"
...
def clear_network_emulation(device):
    # tc-Befehl zum Entfernen aller netem-Einstellungen auf dem Gerät
    command = ["tc", "qdisc", "del", "dev", device, "root"]

    # Ausführen des Befehls
    try:
        result = subprocess.run(command, check=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        print("Filter erfolgreich entfernt.")
    except subprocess.CalledProcessError as e:
        print("Fehler beim Entfernen des Filters:", e.stderr.decode())

# Beispielaufruf zum Entfernen des Filters
clear_network_emulation(device="enp0s25")